...
IF EXISTS(select * from sys.databases where name='yelpClassTable')
  DROP DATABASE yelpClassTable;
go

create database yelpClassTable;
go
...
create table availableFeature (
    ...
    available varchar(5),
    primary key(businessId, feature),
    foreign key (businessId, feature) references businessFeature(businessId, feature)
);
go
...
create table groupedFeature (
    ...
    feature varchar(80),
    available varchar(5),
    g_id int foreign key references featuregroup(id),
    primary key(businessId, feature),
    foreign key (businessId, feature) references businessFeature(businessId, feature)
);
go
